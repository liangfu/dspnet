...
class Detector(object):
    ...
    def detect(self, det_iter, show_timer=False, is_image=True):
        ...
        if self.method==1:
            ...
            results = [self.executor.outputs[3],self.executor.outputs[4]]
            # print('elapsed: %.1f ms' %((timer()-start)*1000.,))
        ...
        segmentation = np.squeeze(mx.nd.argmax(results[1],axis=1).asnumpy())
        ...
        if imgname.endswith(".png") or imgname.endswith(".jpg"):
            ...
            while 1:
                ...
                if self.data_shape[1]==320:
                    img = cv2.resize(img, (640, 320))
                tic0 = time.time()
                dets, seg = self.im_detect_single(img, show_timer=True)
                toc0 = time.time()
                det = dets[0]
                img[:, :, (0, 1, 2)] = img[:, :, (2, 1, 0)]
