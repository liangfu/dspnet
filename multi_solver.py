...
def display_results(out_img,label_img,img,det,gt_boxes,class_names):
    ...
    for box in gt_boxes.tolist():
        ...
        color = palette[det2seg[box[0]],::-1]
        cv2.rectangle(img, (bbox[0], bbox[1]), (bbox[2], bbox[3]), color=tuple(color.tolist()), thickness=thickness)
                ...
                for data, fnames in eval_data:
                    ...
                    if self.evaluation_only:
                        ...
                        for imgidx in range(eval_data.batch_size):
                            ...
                            res_fname = fnames[imgidx].replace("SegmentationClass","Results").replace("gtFine_labelTrainIds","results")
                            lut = np.zeros(256)
                            ...
                            # display result
                            display_img = display_results(res_img,np.expand_dims(label_img,axis=0),img, det, gt, self.class_names)
                            res_fname = fnames[imgidx].replace("SegmentationClass","Results").replace("gtFine_labelTrainIds","compare")
