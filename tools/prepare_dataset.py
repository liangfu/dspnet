...
def load_cityscapes(image_set, dirname, shuffle=False):
    """
    wrapper function for loading cityscapes dataset
    ...
    image_set : str
        train, val
    dirname: str
        root dir for cityscapes
    shuffle: boolean
        initial shuffle
    """
    anno_files = [y.strip() for y in image_set.split(',')]
    assert anno_files, "No image set specified"
    imdbs = []
    for af in anno_files:
        # af_path = os.path.join(dirname, af)
        imdbs.append(Cityscapes(af, dirname, shuffle, is_train=True))
    if len(imdbs) > 1:
        return ConcatDB(imdbs, shuffle)
    else:
        return imdbs[0]
...
if __name__ == '__main__':
    ...
    if args.dataset == 'pascal':
        ...
        db = load_cityscapes(args.set, args.root_path, args.shuffle)
        print("saving list to disk...")
        db.save_imglist(args.target, root=args.root_path)
    else:
    ...
    print("Record file {} generated...".format(os.path.abspath(args.target).split('.')[0] + '.rec'))
