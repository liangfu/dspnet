...
class IoUMetric(mx.metric.EvalMetric):
    """Computes segmentation IoU score.
    ...
    """
    def __init__(self, axis=1, name='mIoU', output_names=None, class_names=None):
        super(IoUMetric, self).__init__(
            name, axis=axis,
            output_names=output_names, class_names=class_names)
        if class_names is None:
            self.num = None
        else:
            assert isinstance(class_names, (list, tuple))
    ...
    def reset(self):
        ...
        if getattr(self, 'num', None) is None:
            ...
            self.num_inst = np.zeros(self.num) # [0] * self.num
            self.sum_metric = np.zeros(self.num) # [0.0] * self.num
        self.records = dict()
        self.counts = dict()
    ...
    def update(self, labels, preds):
        ...
        for label, pred_label in zip(labels, preds):
            ...
            for idx in range(self.num):
                inter = ((label.flat == idx) & (pred_label.flat == idx)).sum()
                total = ((label.flat == idx) | (pred_label.flat == idx)).sum()
                self.sum_metric[idx] += inter
                self.num_inst[idx] += total

    def _update(self):
        self.sum_metric[-1]=np.mean(self.sum_metric[:-1]/(self.num_inst[:-1]+1e-5))
        self.num_inst[-1]=1.
